...
pfQuest.updateThrottleFrame = CreateFrame("Frame")
pfQuest.updateThrottleFrame.delay = 0.25
pfQuest.updateThrottleFrame:SetScript("OnUpdate", function(self, elapsed)
    self.timer = (self.timer or 0) - elapsed
    if self.timer <= 0 then
        self:Hide()
        if self.needsLogUpdate then
            pfQuest:Debug("Update Quest|cff33ffcc Log|r [|cffff3333Throttled|r]")
            pfQuest:UpdateQuestlog()
            self.needsLogUpdate = false
        end
        if self.needsGiverUpdate then
            pfQuest:Debug("Update Quest|cff33ffcc Givers|r [|cffff3333Throttled|r]")
            ...
            self.needsGiverUpdate = false
        end
    end
end)
pfQuest.updateThrottleFrame:Hide()

local function ScheduleUpdate(log, givers)
    if log then pfQuest.updateThrottleFrame.needsLogUpdate = true end
    if givers then pfQuest.updateThrottleFrame.needsGiverUpdate = true end
    pfQuest.updateThrottleFrame.timer = pfQuest.updateThrottleFrame.delay
    pfQuest.updateThrottleFrame:Show()
end
...
pfQuest:SetScript("OnEvent", function(self, event, arg1)
  ...
  if event == "QUEST_LOG_UPDATE" and self.lock and self.lock > GetTime() then
...
pfQuest:SetScript("OnUpdate", function(self)
  ...
  -- check questlog periodically as a fallback
