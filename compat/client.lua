...
if client >= 30000 then -- WotLK and later
  pfQuestCompat.GetPlayerFacing = GetPlayerFacing
else -- Vanilla/TBC fallback
  ...
  for k, v in pairs({Minimap:GetChildren()}) do
    if v:IsObjectType("Model") and not v:GetName() then
      if v:GetModel() and string.find(strlower(v:GetModel()), "interface\\minimap\\minimaparrow") then
        minimaparrow = v
        break
      end
    end
  ...
  pfQuestCompat.GetPlayerFacing = function()
    -- Ensure MiniMapCompassRing and minimaparrow are valid before using them.
    if pfQuestCompat.rotateMinimap then
      ...
      if minimaparrow and minimaparrow.GetFacing then
