  ...
  for addon, data in pairs(pfMap.nodes) do
    ...
    if data[mapID] and minimap_sizes[mapID] and pfMap:HasMinimap(mapID) then
      for coords, node in pairs(data[mapID]) do
        ...
        local display = nil
        local distance = nil
        local halfWidth = pfMap.drawlayer:GetWidth() / 2

        if pfUI and pfUI.minimap then
          display = ( abs(xPos) + 8 < halfWidth and abs(yPos) + 8 < pfMap.drawlayer:GetHeight()/2 )
          if display then
            distance = sqrt(xPos * xPos + yPos * yPos)
          end
        else
          local dist_sq = xPos * xPos + yPos * yPos
          if dist_sq < (halfWidth - 8) * (halfWidth - 8) then
            display = true
