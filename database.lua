...
function pfDatabase:GetIDByName(name, db, partial, server)
  if not pfDB[db] or not name then return {} end
  local ret = {}
  local lower_name
  ...
  for id, loc in pairs(pfDB[db]["loc"]) do
    if db == "quests" then
      if loc then loc = loc["T"] end
    end

    local custom = server and pfQuest_server[db] and pfQuest_server[db][id] or not server
    if loc and custom then
      if partial == true then
        if string.find(string.lower(loc), lower_name, 1, true) then
          ret[id] = loc
        end
      elseif partial == "LOWER" then
        if string.lower(loc) == lower_name then
          ret[id] = loc
        end
...
function pfDatabase:GetQuestIDs(qid)
  ...
  -- First, find all quests with a matching title
  local title_matches = {}
  for id, data in pairs(pfDB["quests"]["loc"]) do
    if quests[id] and data.T == title then
      table.insert(title_matches, id)
    end
  end
  local tcount = #title_matches

  -- No exact title match, fallback to Levenshtein on titles
  if tcount == 0 and title then
    ...
    if not ttitle then
      ...
      title = ttitle
      -- Re-run the title match with the corrected title
      for id, data in pairs(pfDB["quests"]["loc"]) do
        if quests[id] and data.T == title then
          table.insert(title_matches, id)
        end
      end
      tcount = #title_matches
    end
  end

  -- Iterate only over quests that matched the title
  for _, id in ipairs(title_matches) do
    local score = 1 -- Base score for title match
    ...
    if quests[id] then
      ...
      -- Score for matching race
      if quests[id]["race"] and ( bit.band(quests[id]["race"], prace) == prace ) then
        score = score + 8
      end

      -- Score for matching class
      if quests[id]["class"] and ( bit.band(quests[id]["class"], pclass) == pclass ) then
        score = score + 8
      end

      -- If still ambiguous, use Levenshtein on objective and description text
      if tcount > 1 then
        local data = pfDB.quests.loc[id]
        if data then
          if data["O"] then
            score = score + max(24 - lev(pfDatabase:FormatQuestText(data["O"]), objective, 24), 0)
          end
          if data["D"] then
            score = score + max(24 - lev(pfDatabase:FormatQuestText(data["D"]), text, 24), 0)
          end
        end
      end
