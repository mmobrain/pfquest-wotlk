...
local function ResultButtonReload(self)
  ...
  elseif self.btype == "items" then
    ...
    table.insert(pfBrowser.itemUpdateQueue, self)
    pfBrowser.itemUpdateFrame:Show()
  end
...
pfBrowser.searchCache = {}
pfBrowser:SetScript("OnHide", function()
    -- Clear search cache and stop any pending searches
    wipe(pfBrowser.searchCache)
    if pfBrowser.searchThrottleFrame then
      pfBrowser.searchThrottleFrame:Hide()
    end
end)
...
-- Optimized Item Update Handling
pfBrowser.itemUpdateQueue = {}
pfBrowser.itemUpdateFrame = CreateFrame("Frame")
pfBrowser.itemUpdateFrame:SetScript("OnUpdate", function(self, elapsed)
    if #pfBrowser.itemUpdateQueue == 0 then
        self:Hide()
        return
    end

    local button = table.remove(pfBrowser.itemUpdateQueue, 1)

    if not button or not button:IsShown() or button.itemColor then
        return
    end

    GameTooltip:SetOwner(UIParent, "ANCHOR_NONE")
    GameTooltip:ClearLines()
    GameTooltip:SetHyperlink("item:" .. button.id .. pfQuestCompat.itemsuffix)

    local _, _, itemQuality = GetItemInfo(button.id)
    GameTooltip:Hide()
    ...
    if itemQuality then
        ...
        button.itemColor = "|c" .. string.format("ff%02x%02x%02x", r, g, b)

        local custom = pfQuest_server["items"][button.id] and " [|cff33ffcc!|r]" or ""
        button.text:SetText(button.itemColor .."|Hitem:"..button.id..pfQuestCompat.itemsuffix.."|h[".. button.name.."]|h|r"..custom)
        button.text:SetWidth(button.text:GetStringWidth())
    else
        button.refreshCount = (button.refreshCount or 0) + 1
        if button.refreshCount <= 10 then
            table.insert(pfBrowser.itemUpdateQueue, button)
        end
    end
end)
pfBrowser.itemUpdateFrame:Hide()

function pfBrowser:PerformSearch()
  local text = pfBrowser.input:GetText()
  if text == pfQuest_Loc["Search"] then text = "" end

  if pfBrowser.searchCache[text] then
      for _, caption in ipairs({"Units","Objects","Items","Quests"}) do
          local searchType = strlower(caption)
          local data = pfBrowser.searchCache[text][searchType] or {}
          local i = 0
          for id, name in pairs(data) do
              ...
              pfBrowser.tabs[searchType].buttons[i].name = name
  ...
  pfBrowser.searchCache[text] = {}

  local custom = string.find(text, "^custom:")
  local searchText = string.gsub(text, "^custom:", "")

  wipe(pfBrowser.itemUpdateQueue)
  pfBrowser.itemUpdateFrame:Hide()

  for _, caption in ipairs({"Units","Objects","Items","Quests"}) do
      local searchType = strlower(caption)
      local data = (strlen(searchText) >= 3 or custom) and pfDatabase:GetIDByName(searchText, searchType, true, custom) or {}

      pfBrowser.searchCache[text][searchType] = data
...
pfBrowser.searchThrottleFrame = CreateFrame("Frame")
pfBrowser.searchThrottleFrame.delay = 0.25
pfBrowser.searchThrottleFrame.timer = 0
pfBrowser.searchThrottleFrame:SetScript("OnUpdate", function(self, elapsed)
    self.timer = self.timer - elapsed
    if self.timer <= 0 then
        self:Hide()
        pfBrowser:PerformSearch()
    end
end)
pfBrowser.searchThrottleFrame:Hide()
...
-- This script now throttles searches instead of running them on every keystroke
pfBrowser.input:SetScript("OnTextChanged", function(self)
  ...
  if strlen(searchText) < 3 and not custom then
      pfBrowser.searchThrottleFrame:Hide()
      wipe(pfBrowser.searchCache)

      for _, caption in ipairs({"Units","Objects","Items","Quests"}) do
          local searchType = strlower(caption)
          local data = pfBrowser_fav[searchType]
          local i = 0
          for id, name in pairs(data) do
            i = i + 1
            if i >= search_limit then break end
  ...
  pfBrowser.searchThrottleFrame.timer = pfBrowser.searchThrottleFrame.delay
  pfBrowser.searchThrottleFrame:Show()
end)
